...
int changyy_io(char *wbuf, int *wlen, char *rbuf, int *rlen, int iseof,
            ci_request_t * req);
...
int changyy_init_service(ci_service_xdata_t * srv_xdata, struct ci_server_conf *server_conf)
{
	ci_debug_printf(5, "Initialization of changyy module......\n");
...
void *changyy_init_request_data(ci_request_t * req)
{
	...
	ci_debug_printf(5, "changyy_init_request_data\n");
...
void changyy_release_request_data(void *data)
{
	...
	ci_debug_printf(5, "changyy_release_request_data\n");
...
int changyy_check_preview_handler(char *preview_data, int preview_data_len, ci_request_t * req)
{
	...
	ci_debug_printf(5, "changyy_check_preview_handler\n");
	...
	if( (value = ci_http_request_get_header(req,"Cookie") ) )
		ci_debug_printf(5, "Cookie: %s\n", value);
	if( (value = ci_http_request_get_header(req,"Content-Type") ) )
		ci_debug_printf(5, "Content-Type: %s\n", value);
	
	int whattodo = 0;
	if (whattodo == 0) {
		ci_debug_printf(8, "changyy service will process the request\n");
...
{
    struct changyy_req_data *changyy_data = ci_service_data(req);
    ci_debug_printf(5, "changyy_end_of_data_handler\n");
    /*mark the eof*/
    changyy_data->eof = 1;
            ...
            ci_request_t * req)
{
     int ret;
     struct changyy_req_data *changyy_data = ci_service_data(req);
     ci_debug_printf(5, "changyy_io, finish:%d, buffer length:%d\n", iseof, *rlen);
     ret = CI_OK;
     ...
     /*read some data from the changyy_data->body and put them to the write buffer to be send
      to the ICAP client*/
     if (wbuf && wlen) {
